...
parser.add_argument('--data_set', default='CUB2011', 
    choices=['CUB2011U', 'Car', 'Dogs', 'CUB2011','Poke'], type=str)
parser.add_argument('--data_path', type=str, default='./datasets/archive/')
...
parser.add_argument('--prototype_shape', nargs='+', type=int, default=[2000, 64, 150, 1])
...
train_dataset = datasets.ImageFolder(
    os.path.join(args.data_path, 'PokemonDataTrain'),
    transforms.Compose([
...
test_dataset = datasets.ImageFolder(
    os.path.join(args.data_path, 'PokemonDataTest'),
    transforms.Compose([
        transforms.Resize(size=(img_size, img_size)),
        transforms.ToTensor(),
        normalize,
...
final_cm_path = output_dir / 'poke_confusion_matrix.png'
plt.savefig(final_cm_path)
plt.close()
logger.info(f'Saved final confusion matrix at {final_cm_path}')
